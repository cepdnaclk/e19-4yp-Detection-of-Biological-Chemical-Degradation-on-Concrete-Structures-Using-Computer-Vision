...
model = models.Model(inputs=[image_input, attack_input], outputs=output)
model.compile(optimizer="adam", loss="mse", metrics=["mae"])

history = model.fit(
    [X_img_train, X_attack_train],
    y_train,
    validation_data=([X_img_test, X_attack_test], y_test),
    epochs=30,
    batch_size=8,
)
plot_mae_history(history, "DenseNet121 (Frozen)")
...
plot_mae_history(fine_history, "DenseNet121 (Fine-tuned)")
...
model.save("saved_models/densenet_attack_predictor.h5")
joblib.dump(scaler, "saved_models/target_scaler.pkl")
